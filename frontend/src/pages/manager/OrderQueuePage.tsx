...
const ACTION_FN: Record<string, (id: string) => Promise<Order>> = {
                  ...
                  <div key={order.id} className="bg-white rounded-lg p-3 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {col.actions.map(action => (
                        <button
                          key={action}
                          onClick={() => doAction({ action, id: order.id })}
          ...
          {orders
            ...
            .map((order: Order) => (
              <div key={order.id} className="bg-white rounded-xl border p-4 flex items-center justify-between">
                <div>
                  <span className="text-xs text-gray-400 mr-2">{order.id.slice(0, 8)}…</span>
